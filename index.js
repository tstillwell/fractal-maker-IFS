...
function promptForIFS(){ // prompt control flow
  ...
  switch (isValidEquation(line)){
    case true: // add this input to equations array
...
function generateNextPoint(point, IFSparams){
  ...
  const next_x = IFSparams[0] * point[0] + IFSparams[1] * point[1] + IFSparams[4];
  const next_y = IFSparams[2] * point[0] + IFSparams[3] * point[1] + IFSparams[5];
  const next_point = [next_x, next_y];
  return next_point;
}
